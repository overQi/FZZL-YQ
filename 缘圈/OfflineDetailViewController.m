...
- (NSString *)cellDetailTextForItem:(MAOfflineItem *)item
{
    ...
    if (![self.downloadingItems containsObject:item])
    {
        ...
        {
            detailText = [NSString stringWithFormat:@"%.2fMB/%.2fMB", (float)item.downloadedSize/1024/1024, (float)item.size/1024/1024];
        }else{
            detailText = [NSString stringWithFormat:@"大小:%.2fMB", (float)item.size/1024/1024];
        }
    }
    else
    {
        ...
        switch (status)
        {
            ...
            case MAOfflineMapDownloadStatusProgress:
            {
                ...
                detailText = [NSString stringWithFormat:@"%.2fMB/%.2fMB(%.2f%%)", (float)recieved/1024/1024, (float)expected/1024/1024, recieved/(float)expected*100];
                break;
            }
...
- (void)searchAction
{
    ...
    FZ_LOG(@"key = %@, result count = %ld", key, (unsigned long)result.count);
    [result enumerateObjectsUsingBlock:^(MAOfflineCity *obj, NSUInteger idx, BOOL *stop) {
        FZ_LOG(@"idx = %ld, cityName = %@, cityCode = %@, adCode = %@, pinyin = %@, jianpin = %@, size = %lld", (unsigned long)idx, obj.name, obj.cityCode,obj.adcode, obj.pinyin, obj.jianpin, obj.size);
    }];
}
...
- (void)setupTitle
{
    self.navigationItem.title = @"离线地图管理";
}
